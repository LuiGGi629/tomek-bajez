...
quiver_params = {"angles": "xy", "scale_units": "xy", "scale": 1, "width": 0.012}
...
@set_rc
def plot_transformation_helper(
    axis, matrix, *vectors, unit_vector=True, unit_circle=False, title=None
):
    ...
    *vectors : class numpy.ndarray.
    ...
    if vectors:
        for vector in vectors:
            ...
            axis.quiver(
                origin, origin, [vector_[0]], [vector_[1]], color=color, **quiver_params
            )
...
@set_rc
def plot_3d_transformation_helper(
    axis, matrix, grid=True, unit_sphere=False, title=None
):
...
def plot_3d_linear_transformation(matrix, grid=True, unit_sphere=False):
    ...
    plot_3d_transformation_helper(
        axis2, matrix, grid=grid, unit_sphere=unit_sphere, title="after transformation"
    )
    ...
    for i in range(nplots):  # fig_idx
        ...
        if i == 0:
            ...
            if i == 1:
                ...
                title = "After {} transformations".format(i)
        plot_3d_transformation_helper(
            axis, matrix_trans, grid=grid, unit_sphere=unit_sphere, title=title
        )
    ...
    *vectors : class numpy.ndarray.
    ...
    if vectors:
        for vector in vectors:
            ...
            axis.quiver(
                origin, origin, [vector[0]], [vector[1]], color=red, **quiver_params
            )
...
def plot_change_basis(I, J, *vectors):
    ...
    *vectors : class numpy.ndarray.
    ...
    plot_basis_helper(
        axis2, I, J, *vectors_, title="new basis", I_label="a", J_label="b"
    )
...
def plot_eigen(matrix):
    ...
    plot_basis_helper(
        ...
        numpy.array([1, 0]),
        numpy.array([0, 1]),
        title=r"coords in standard basis $\mathbf{x}$",
    )
    plot_basis_helper(
        axes[0, 1],
        C[:, 0],
        C[:, 1],
        title=r"change to new basis $C^{-1}\mathbf{x}$",
        I_label="a",
        J_label="b",
    )
    plot_basis_helper(
        ...
        C[:, 0],
        C[:, 1],
        title=r"scale along new basis $DC^{-1}\mathbf{x}$",
        I_label="a",
        J_label="b",
    )
    plot_basis_helper(
        ...
        numpy.array([1, 0]),
        numpy.array([0, 1]),
        title=r"change back to standard basis $CDC^{-1}\mathbf{x}$",
    )
